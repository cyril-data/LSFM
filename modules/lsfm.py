...
def my_init(shape, nb_action, dtype=None):
    ...
    Fa_reshape = tf.Variable(tf.reshape(
        F, (F.shape[0], nb_action, F.shape[0])))
...
#         LSFM must be train on policy independant of state
        ...
        layers_ouputs["Fa"] = layers.Reshape(
            (self._action_size, layer_latent.shape[-1]))(x)

#         classification on rewards
        x = layers.Dense(self._action_size * (len(self.reward_parser)+2),
                         use_bias=False, name="ra")(layer_latent)
...
#         var_actions =  tf.stack(aggr, axis=0)
        ...
        var_actions = tf.map_fn(fn=lambda t: var_all_a[t[0], t[1]],
                                elems=[tf.range(actions.shape[0]), actions], dtype=tf.float32)
...
#             psi_bar = self.psi_bar_prev(next_states, action_space, model, model_prev
    ...
    def losses_all(self, states, actions, rewards, next_states, target_psi, model):
        ...
        loss_tot = alpha_r * loss_r + alpha_N * loss_N + \
            alpha_psi * loss_psi + alpha_phi * loss_phi
...
#         print("grads", grads)
        ...
        self.param["optimizer_LSFM"].apply_gradients(
...
#                 )
    ...
    def next_phi_pred(self, model, phis, actions):
        ...
        Ma_reshape = tf.reshape(
            Ma, (Ma.shape[0], self._action_size, Ma.shape[0]))

        agrr = []

        for k, action in enumerate(actions):
            agrr.append(tf.reshape(
                (tf.matmul(tf.reshape(phis[k], (1, -1)), Ma_reshape[:, action, :])), (-1)))
    ...
    def rewards_latent(self, phi, model):
        ...
        for k in range(var_all_a_reshape.shape[0]):
            ...
            out.append(Lambda(lambda x: tf.keras.activations.softmax(
                x, axis=2))(var_all_a_reshape[k]))
    ...
    def F_bar(self, model):
        for idl, layer in enumerate(model.layers):
            ...
            if layer.name == name_a:
                Fa = layer.weights[0]
        Fa_reshape = tf.reshape(
            Fa, (Fa.shape[0], self._action_size, Fa.shape[0]))
...
#         print("next_phi_pred : ", np.array(phisp1))
    ...
    def tf_random_choice(self, action_available):
        cat = tf.constant(1., shape=(
            1, action_available.shape[0]), dtype=tf.float32)
    ...
    def one_step_greedy_eigenpurpose(self, model, phis, mask_action,  eigvalue=0, negation_vect=False):
        ...
        actions = tf.map_fn(
            fn=lambda t: tf.constant(t, shape=(phis.shape[0])), elems=action_available, dtype=tf.int32)

        next_possible_phis = tf.map_fn(
            fn=lambda t:  self.intrinsec_rewards(
                model, phis, t, eigvalue, negation_vect), elems=actions, dtype=tf.float32)
...
#         print("intrisec_reward_max", intrisec_reward_max)
        ...
        terminate = tf.where(intrisec_reward_max <= 0.)[:, 0]
        terminate = tf.cast(terminate, tf.int32)

        # random.choice(action_available)    ==     tf_random_choice(action_available)

        action_greedy_mask = tf.map_fn(fn=lambda t: self.action_greedy_mask_fct(
            t, terminate, action_available, action_greedy), elems=tf.range(
            action_greedy.shape[0]), dtype=tf.int32)

#         action_greedy_mask

        self.eigenoption = tf.map_fn(
            fn=lambda t: self.eigenoption_fct(
                t, terminate, self.eigenoption), elems=tf.range(action_greedy.shape[0]), dtype=tf.int32)
        ...
        if self.eigenoption[0] == -1:
            ...
            for k in range(phis.shape[0]):
                actions.append(random.choice(
                    np.where(np.array(mask_action) > 0)[0]))

            if (self.policy == "eps-greedy"):
                if (self.param["policy"]["type"][1] == "exponantial"):
                    self.eigen_exploration = self.eps_min + (self.eps_max - self.eps_min) * \
                        math.exp(-self._lambda * steps)


#           Once the random action is taken, choose between a option or random action (1-ei_expl / ei_expl)
            eigenchoice = random.choices(
                [0, 1], weights=(self.eigen_exploration, 1. - self.eigen_exploration))[0]
#           if self.eigen_exploration --> 1  => eigenchoice --> 0

            if eigenchoice == 1:  # next action will be an eigen option
                if self.eigenoption_number > 1:  # self.eigenoption_number = 0 is to force random action
                    self.eigenoption = tf.reshape(self.tf_random_choice(
                        tf.range(self.eigenoption_number - 1)), (1))
...
#             print("action optiondiscovery", np.array(actions))
    ...
    def visu_grid(self, visu_state, grid_size, axs):
        ...
        axs.set_xticks(tf.range(-.5, grid_size - 1, 1), minor=True)
        axs.set_yticks(tf.range(-.5, grid_size - 1, 1), minor=True)
